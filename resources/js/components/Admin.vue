<template>
  <div class="container" id="inside-container">
    <h3 class="text-center change-color">Please, select coffee</h3>
    ...
    <div class="card bg-light mb-3" v-for="coffee in getCoffeeToReview" :key="coffee.id">
      ...
      <div class="card-body row">
        ...
        <div class="col-8 col-xl-11">
          <h5>
            <span class="change-color">City:</span>
            {{coffee.city}}
          </h5>
          <h5>
            <span class="change-color">Country:</span>
            {{coffee.country}}
          </h5>
        </div>
      </div>
  ...
  methods: {
    ...
    rejectContribution() {
      if (this.checkedCoffees.length == 0) {
        ...
        if (
          confirm("Are you sure you want to reject these coffee from database")
        ) {
          axios
            .post("/deleteCoffee", { reviewedCoffee: this.checkedCoffees })
            .then(response => {
              this.message = response.data;
              this.checkedCoffees = [];
            })
